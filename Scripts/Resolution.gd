...
func _on_res_dropdown_pressed():
	...
	TL_Node.resolution_dropdown.get_popup().set_position(Vector2(le_x_pos_x, (le_x_pos_y + le_x_size_y) ))
	TL_Node.resolution_dropdown.get_popup().rect_size.x = (le_x_size_x + le_multi_size_x + le_y_size_x + drp_size_x)


func res_load_values():
	var file_x = File.new() #new file class on which you will call file class methods
#	var the_path = path.resolution_x
#	print("PATH.RESOLUTION_X: ", path.resolution("x"))
#	print("ADDRESS IS: ", path.name)
