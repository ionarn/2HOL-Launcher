...
func _on_gcb_checkbutton_toggled(button_pressed):
	...
	hetuw_lines_array[parsed_line] = key_name + " = " + str(int(button_pressed)) + TL_Variables.hetuw_setting_list[hetuw_setting_index]["info"]
...
func reset_button_visibility():
	...
	if hetuw_lines_array[parsed_line] == key_name + " = 1" + TL_Variables.hetuw_setting_list[hetuw_setting_index]["info"]:
		$hcb_reset/reset.visible == true
