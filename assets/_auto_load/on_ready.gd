...
func _ready():
	...
	if file.file_exists(game_file_path) == true: # NEEDS TO BE SET TO FALSE!!! (When OneLife.exe/OneLifeApp exists)
		TL_Node.pop_popup_background.visible = true
		TL_Node.pop_wrong_directory.popup()
	elif file.file_exists(game_file_path) == false: # NEEDS TO BE SET TO ELIF AND TRUE!!! (When OneLife.exe/OneLifeApp exists)
